  ...
  return (
    <div className="w-full px-4 sm:px-6 lg:px-8 py-8 relative">
      <div className="sm:flex sm:items-center sm:justify-between mb-8">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Tariflar va Obunalar</h1>
          <p className="mt-2 text-sm text-gray-700">Platformangizdagi obuna narxlari va imkoniyatlarini boshqaring</p>
        </div>
          ...
          <div key={plan.id} className={`rounded-2xl border ${plan.isActive ? 'border-blue-500 shadow-md' : 'border-gray-200 opacity-70'} bg-white p-6 flex flex-col transition-all relative overflow-hidden group`}>
            <div className="mb-4">
              ...
              <p className="mt-2 text-3xl font-extrabold text-[#1E40AF]">
                {Number(plan.price) === 0 ? "Tekin" : `${Number(plan.price).toLocaleString()} so'm`}
              </p>
            ...
            <div className="space-y-2">
              ...
              >
                {plan.isActive ? "Tarifni o'chirish" : "Tarifni faollashtirish"}
              </button>
              <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-fade-in-up">
            <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
              <h3 className="text-lg font-bold text-gray-900">{editingPlan ? "Tarifni tahrirlash" : "Yangi Tarif"}</h3>
              <button onClick={() => setIsModalOpen(false)} className="text-gray-400 hover:text-gray-600 font-bold text-xl">&times;</button>
            </div>
            <form onSubmit={handleSubmit} className="p-6">
              <div className="mb-4">
                <label className="block text-sm font-medium text-gray-700 mb-1">Tarif Nomi</label>
                <input required type="text" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-[#1E40AF]" placeholder="Masalan: VIP Tarif" />
              </div>
              <div className="mb-4">
                <label className="block text-sm font-medium text-gray-700 mb-1">Narxi (so'mda)</label>
              ...
              <div className="mb-6">
                <label className="block text-sm font-medium text-gray-700 mb-1">Imkoniyatlari (Har bir qator alohida imkoniyat)</label>
                <textarea required rows="4" value={formData.features} onChange={(e) => setFormData({...formData, features: e.target.value})} className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-[#1E40AF]" placeholder="Barcha kurslarga kirish&#10;Sertifikat beriladi..."></textarea>
              </div>
